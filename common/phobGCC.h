...
ControlConfig _controls{
    ...
    .rumbleMin = 0,
    .rumbleMax = 11,
    .rumbleDefault = 9, // 5 is the max for 3v cell rumble, 9 is for oem-feeling
                        // normal rumble motors
    .rumbleFactory = 9,
    .safeMode = true,
    .autoInit = false,
    .lTrigInitial = 0,
...
FilterGains _gains{
    // these values are for 800 hz, recomputeGains converts them to what is
    // needed for the actual frequency (1000 Hz)
    .maxStick = 100,       .xVelDecay = 0.1,      .yVelDecay = 0.1,
    .xVelPosFactor = 0.01, .yVelPosFactor = 0.01, .xVelDamp = 0.125,
    .yVelDamp = 0.125,     .velThresh = 1.00,     .accelThresh = 3.00,
    .xSmoothing = 0.0,     .ySmoothing = 0.0,     .cXSmoothing = 0.0,
    .cYSmoothing = 0.0};
FilterGains _normGains; // this gets filled by recomputeGains();

Pins _pinList{.pinLa = _pinLa,
              .pinRa = _pinRa,
              .pinL = _pinL,
              .pinR = _pinR,
              .pinAx = _pinAx,
...
int calcRumblePower(const int rumble) {
  if (rumble > 0) {
    return pow(2.0, 7 + ((rumble - 3) / 8.0)); // should be 256 when rumble is
                                               // 11
  } else {
    return 0;
  }
...
void freezeSticksToggleIndicator(const int time, Buttons &btn,
                                 Buttons &hardware, bool toggle) {
...
// This clears all the buttons but doesn't overwrite the sticks or shoulder
// buttons.
...
void showRumble(const int time, Buttons &btn, Buttons &hardware,
...
void changeRumble(const Increase increase, Buttons &btn, Buttons &hardware,
                  ControlConfig &controls) {
...
void adjustSnapback(const WhichAxis axis, const Increase increase, Buttons &btn,
                    Buttons &hardware, ControlConfig &controls,
                    FilterGains &gains, FilterGains &normGains) {
...
void adjustWaveshaping(const WhichStick whichStick, const WhichAxis axis,
                       const Increase increase, Buttons &btn, Buttons &hardware,
                       ControlConfig &controls) {
  debug_println("adjusting waveshaping");
  if (whichStick == ASTICK) {
    if (axis == XAXIS) {
      if (increase == INCREASE) {
        controls.axWaveshaping =
            min(controls.axWaveshaping + 1, controls.waveshapingMax);
      } else {
        controls.axWaveshaping =
            max(controls.axWaveshaping - 1, controls.waveshapingMin);
      }
    } else { // y axis
      if (increase == INCREASE) {
        controls.ayWaveshaping =
            min(controls.ayWaveshaping + 1, controls.waveshapingMax);
      } else {
        controls.ayWaveshaping =
            max(controls.ayWaveshaping - 1, controls.waveshapingMin);
      }
    }
    ...
    if (axis == XAXIS) {
      if (increase == INCREASE) {
        controls.cxWaveshaping =
            min(controls.cxWaveshaping + 1, controls.waveshapingMax);
      } else {
        controls.cxWaveshaping =
            max(controls.cxWaveshaping - 1, controls.waveshapingMin);
      }
    } else {
      if (increase == INCREASE) {
        controls.cyWaveshaping =
            min(controls.cyWaveshaping + 1, controls.waveshapingMax);
      } else {
        controls.cyWaveshaping =
            max(controls.cyWaveshaping - 1, controls.waveshapingMin);
      }
    }
...
void adjustSmoothing(const WhichAxis axis, const Increase increase,
                     Buttons &btn, Buttons &hardware, ControlConfig &controls,
                     FilterGains &gains, FilterGains &normGains) {
...
void showAstickSettings(Buttons &btn, Buttons &hardware,
                        const ControlConfig &controls, FilterGains &gains) {
  // Snapback on A-stick
...
void adjustCstickSmoothing(const WhichAxis axis, const Increase increase,
                           Buttons &btn, Buttons &hardware,
                           ControlConfig &controls, FilterGains &gains,
                           FilterGains &normGains) {
...
void adjustCstickOffset(const WhichAxis axis, const Increase increase,
                        Buttons &btn, Buttons &hardware,
                        ControlConfig &controls) {
...
void showCstickSettings(Buttons &btn, Buttons &hardware,
                        ControlConfig &controls, FilterGains &gains) {
...
void adjustCardinalSnapping(const WhichStick whichStick,
                            const Increase increase, Buttons &btn,
                            Buttons &hardware, ControlConfig &controls) {
...
void adjustAnalogScaler(const WhichStick whichStick, const Increase increase,
                        Buttons &btn, Buttons &hardware,
                        ControlConfig &controls) {
  if (whichStick == ASTICK && increase == INCREASE) {
...
void adjustTriggerOffset(const WhichTrigger trigger, const Increase increase,
                         Buttons &btn, Buttons &hardware,
                         ControlConfig &controls) {
...
void changeTournamentToggle(Buttons &btn, Buttons &hardware,
                            ControlConfig &controls) {
...
void applyJump(const ControlConfig &controls, const Buttons &hardware,
               Buttons &btn) {
...
void toggleExtra(ExtrasSlot slot, Buttons &btn, Buttons &hardware,
                 ControlConfig &controls) {
...
void configExtra(ExtrasSlot slot, Buttons &btn, Buttons &hardware,
                 ControlConfig &controls) {
...
bool checkAdjustExtra(ExtrasSlot slot, Buttons &btn, bool checkConfig) {
  // Extras Toggles: Both control sticks as Up, Down, Left, or Right, and with A
  // + B
  if (!checkConfig) {
    switch (slot) {
    case EXTRAS_UP:
...
void nextTriggerState(WhichTrigger trigger, Buttons &btn, Buttons &hardware,
                      ControlConfig &controls) {
  if (trigger == LTRIGGER) {
  ...
  // if the modes are incompatible due to mode 5, make it show -100 on the stick
  // that isn't mode 5 (user-facing mode 5)
...
void initializeButtons(const Pins &pin, Buttons &btn, int &startUpLa,
                       int &startUpRa) {
  // set the analog stick values to the chosen center value that will be
  // reported to the console on startup We choose 127 (_intOrigin) for this, and
  // elsewhere we use an offset of 127.5 (_floatOrigin) truncated to int in
  // order to round properly
  btn.Ax = _intOrigin;
  btn.Ay = _intOrigin;
  btn.Cx = _intOrigin;
  btn.Cy = _intOrigin;

  // read the ADC inputs for the analog triggers a few times and choose the
  // startup value to be the maximum that was recorded these values could be
  // used as offsets to set particular trigger values
  startUpLa = 0;
  startUpRa = 0;
  for (int i = 0; i < 64; i++) {
...
void applyCalFromPoints(const WhichStick whichStick, float notchAngles[],
                        const float tempCalPointsX[],
                        const float tempCalPointsY[], NotchStatus notchStatus[],
                        float measuredNotchAngles[], StickParams &stickParams,
                        const ControlConfig &controls) {
  // recall previous notch angles
  getNotchAnglesSetting(notchAngles, whichStick);
  // make temp temp cal points that are missing all tertiary notches so that we
  // get a neutral grid
  float temptempCalPointsX[_noOfCalibrationPoints];
  float temptempCalPointsY[_noOfCalibrationPoints];
  stripCalPoints(tempCalPointsX, tempCalPointsY, temptempCalPointsX,
                 temptempCalPointsY);
  ...
  cleanCalPoints(temptempCalPointsX, temptempCalPointsY, _notchAngleDefaults,
                 cleanedPointsX, cleanedPointsY, notchPointsX, notchPointsY,
                 notchStatus);
  linearizeCal(cleanedPointsX, cleanedPointsY, cleanedPointsX, cleanedPointsY,
               stickParams);
  notchCalibrate(cleanedPointsX, cleanedPointsY, notchPointsX, notchPointsY,
                 _noOfNotches, stickParams);
  // apply the calibration to the original measured values including any
  // tertiaries; we don't care about the angles
  cleanCalPoints(tempCalPointsX, tempCalPointsY, _notchAngleDefaults,
                 cleanedPointsX, cleanedPointsY, notchPointsX, notchPointsY,
                 notchStatus);
  float transformedX[_noOfNotches + 1];
  float transformedY[_noOfNotches + 1];
  transformCalPoints(cleanedPointsX, cleanedPointsY, transformedX, transformedY,
                     stickParams, controls, whichStick);
  // compute the angles for those notches into measuredNotchAngles, using the
  // default angles for the diagonals
  computeStickAngles(transformedX, transformedY, measuredNotchAngles);
  // clean full cal points again, feeding those angles in
  cleanCalPoints(tempCalPointsX, tempCalPointsY, measuredNotchAngles,
                 cleanedPointsX, cleanedPointsY, notchPointsX, notchPointsY,
                 notchStatus);
  // clear unused notch angles
  cleanNotches(notchAngles, measuredNotchAngles, notchStatus);
  // clean full cal points again again, feeding those measured angles in for
  // missing tertiary notches
  cleanCalPoints(tempCalPointsX, tempCalPointsY, notchAngles, cleanedPointsX,
                 cleanedPointsY, notchPointsX, notchPointsY, notchStatus);
  // linearize again
  linearizeCal(cleanedPointsX, cleanedPointsY, cleanedPointsX, cleanedPointsY,
               stickParams);
...
int readEEPROM(ControlConfig &controls, FilterGains &gains,
               FilterGains &normGains, StickParams &aStickParams,
               StickParams &cStickParams, const bool noLock = false) {
  int numberOfNaN = 0;
...
void resetDefaults(HardReset reset, ControlConfig &controls, FilterGains &gains,
  ...
  if (reset == HARD || reset == FACTORY) {
    ...
    cleanCalPoints(_defaultCalPointsX, _defaultCalPointsY, notchAngles,
                   cleanedPointsX, cleanedPointsY, notchPointsX, notchPointsY,
                   notchStatus);
...
void calibrationSkipMeasurement(
    int &currentCalStep, const WhichStick whichStick, float tempCalPointsX[],
    float tempCalPointsY[], NotchStatus notchStatus[], float notchAngles[],
    float measuredNotchAngles[], StickParams &aStickParams,
    StickParams &cStickParams, const ControlConfig &controls) {
  currentCalStep = _noOfCalibrationPoints;
  // Do the same thing we would have done at step 32 had we actually collected
  // the points, but with stored tempCalPoints
  if (whichStick == CSTICK) {
    ...
    applyCalFromPoints(whichStick, notchAngles, tempCalPointsX, tempCalPointsY,
                       notchStatus, measuredNotchAngles, cStickParams,
                       controls);
    ...
    getPointsSetting(tempCalPointsY, whichStick, YAXIS);
    applyCalFromPoints(whichStick, notchAngles, tempCalPointsX, tempCalPointsY,
                       notchStatus, measuredNotchAngles, aStickParams,
                       controls);
  }
}

void calibrationUndo(int &currentCalStep, const WhichStick whichStick,
  ...
  if (currentCalStep % 2 == 0 && currentCalStep < 32 && currentCalStep != 0) {
    ...
    currentCalStep -= 2;
  } else if (currentCalStep % 2 == 1 && currentCalStep < 32 &&
             currentCalStep != 0) {
  ...
  if (whichStick == CSTICK) {
    int notchIndex = _notchAdjOrder[min(
        currentCalStep - _noOfCalibrationPoints,
        _noOfAdjNotches -
            1)]; // limit this so it doesn't access outside the array bounds
    while (
        (currentCalStep >= _noOfCalibrationPoints) &&
        (notchStatus[notchIndex] == TERT_INACTIVE) &&
        (currentCalStep <
         _noOfCalibrationPoints +
             _noOfAdjNotches)) { // this non-diagonal notch was not calibrated
      // skip to the next valid notch
      currentCalStep--;
      notchIndex = _notchAdjOrder[min(
          currentCalStep - _noOfCalibrationPoints,
          _noOfAdjNotches -
              1)]; // limit this so it doesn't access outside the array bounds
    }
...
void calibrationAdvance(ControlConfig &controls, int &currentCalStep,
                        const WhichStick whichStick, float tempCalPointsX[],
                        float tempCalPointsY[], bool &undoCal,
                        float notchAngles[], NotchStatus notchStatus[],
                        float measuredNotchAngles[], StickParams &aStickParams,
                        StickParams &cStickParams) {
  if (whichStick == CSTICK) {
    if (currentCalStep < _noOfCalibrationPoints) { // still collecting points
      ...
      X /= 128.0;
      Y /= 128.0;
      insertCalPoints(whichStick, currentCalStep, tempCalPointsX,
                      tempCalPointsY, _pinList, X, Y);
    }
    currentCalStep++;
    if (currentCalStep >= 2 &&
        currentCalStep != _noOfNotches * 2) { // don't undo at the beginning of
                                              // collection or notch adjust
      undoCal = true;
    } else {
      undoCal = false;
    }
    if (currentCalStep == _noOfCalibrationPoints) { // done collecting points
      // bring all notches into a legal range; this helps recover from freakout
      // situations
      legalizeNotches(currentCalStep, measuredNotchAngles, notchAngles,
                      notchStatus);
      applyCalFromPoints(whichStick, notchAngles, tempCalPointsX,
                         tempCalPointsY, notchStatus, measuredNotchAngles,
                         cStickParams, controls);
    }
    int notchIndex = _notchAdjOrder[min(
        currentCalStep - _noOfCalibrationPoints,
        _noOfAdjNotches -
    ...
    if (currentCalStep >=
        _noOfCalibrationPoints + _noOfAdjNotches) { // done adjusting notches
      ...
      float notchPointsX[_noOfNotches + 1];
      float notchPointsY[_noOfNotches + 1];
      cleanCalPoints(tempCalPointsX, tempCalPointsY, notchAngles,
                     cleanedPointsX, cleanedPointsY, notchPointsX, notchPointsY,
                     notchStatus);
      debug_println("calibration points cleaned");
      linearizeCal(cleanedPointsX, cleanedPointsY, cleanedPointsX,
                   cleanedPointsY, cStickParams);
      debug_println("C stick linearized");
      notchCalibrate(cleanedPointsX, cleanedPointsY, notchPointsX, notchPointsY,
                     _noOfNotches, cStickParams);
      currentCalStep = -1;
    ...
    if (currentCalStep >= 2 &&
        currentCalStep !=
            _noOfCalibrationPoints) { // don't undo at the beginning of
                                      // collection or notch adjust
      undoCal = true;
    } else {
      undoCal = false;
...
void processButtons(Pins &pin, Buttons &btn, Buttons &hardware,
                    ControlConfig &controls, FilterGains &gains,
                    FilterGains &normGains, int &currentCalStep, bool &running,
                    ...
                    bool *multishine2, uint8_t *multishineSeq2,
                    uint32_t *multishineTimer2) {
  ...
  const bool nowMultishining2 = tempBtn.Dl >= 1;
  ...
  if (!(*multishine2) && nowMultishining2) {
    *multishine2 = nowMultishining2;
    *multishineSeq2 = 0;
    *multishineTimer2 = micros();
  }

  // Swap buttons here for jump remapping
  applyJump(controls, hardware, tempBtn);

  // read the L and R sliders here instead of readSticks so we don't get race
  // conditions for mode 6
  ...
  const bool lockoutL =
      controls.rConfig == 4 &&
      (controls.lConfig != 1 && controls.lConfig != 4 && controls.lConfig != 5);
  const bool lockoutR =
      controls.lConfig == 4 &&
      (controls.rConfig != 1 && controls.rConfig != 4 && controls.rConfig != 5);

  // We multiply the analog trigger reads by this to shut them off if the
  // trigger is mapped to jump
  const int shutoffLa =
      (controls.jumpConfig == SWAP_XL || controls.jumpConfig == SWAP_YL) ? 0
                                                                         : 1;
  const int shutoffRa =
      (controls.jumpConfig == SWAP_XR || controls.jumpConfig == SWAP_YR) ? 0
                                                                         : 1;

  // These are used for mode 7, but they're calculated out here so we can scale
  // the deadzone too.
  ...
  case 3: // Trigger Plug Emulation state
    tempBtn.La =
        (uint8_t)fmin(controls.lTriggerOffset,
                      readLa(pin, controls.lTrigInitial, 1) * shutoffLa);
    break;
  ...
  case 6: // Scales Analog Trigger Values
    tempBtn.La =
        (uint8_t)readLa(pin, controls.lTrigInitial, triggerScaleL) * shutoffLa;
    break;
  default:
    tempBtn.La = (uint8_t)readLa(pin, controls.lTrigInitial, 1) * shutoffLa;
  }
  ...
  case 3: // Trigger Plug Emulation state
    tempBtn.Ra =
        (uint8_t)fmin(controls.rTriggerOffset,
                      readRa(pin, controls.rTrigInitial, 1) * shutoffRa);
    break;
  case 4: // Digital => Analog Value state
    if (tempBtn.R) {
  ...
  case 6: // Scales Analog Trigger Values
    tempBtn.Ra =
        (uint8_t)readRa(pin, controls.rTrigInitial, triggerScaleR) * shutoffRa;
    break;
  default:
    tempBtn.Ra = (uint8_t)readRa(pin, controls.rTrigInitial, 1) * shutoffRa;
  }
  ...
  if ((controls.tournamentToggle == 1 || controls.tournamentToggle == 4) &&
      hardware.Du) {
    if (duLockout > 0) {
      duLockout--;
   ...
   * Adjust by holding both sticks in the chosen direction, then pressing A+Dpad
   * directions
   */
  ...
  // If we enter the following if else block with a nonzero counter but no
  // commands are used,
  ...
  if (!controls.safeMode && (currentCalStep == -1)) {
    ...
    if (hardware.A && hardware.B && hardware.Z &&
        hardware.Dd) {            // Hard Reset pressed
    ...
    if (hardware.A && hardware.X && hardware.Y && hardware.S && !hardware.L &&
        !hardware.R) { // Safe Mode Toggle
      controls.safeMode = true;
      freezeSticks(4000, btn, hardware);
    } else if (hardware.A && hardware.Z && hardware.Du && !hardware.X &&
               !hardware.Y && !hardware.L &&
               !hardware.R) { // display version number (ignore commands for c
                              // stick snapback)
      ...
      clearButtons(2000, btn, hardware);
    } else if (hardware.A && hardware.B && hardware.Z &&
               hardware.S) { // Soft Reset
      resetDefaults(SOFT, controls, gains, normGains, _aStickParams,
                    _cStickParams); // don't reset sticks
      freezeSticks(2000, btn, hardware);
    } else if (hardware.A && hardware.B && hardware.Z &&
               hardware.Dd) { // Hard Reset
      // actually do nothing, this is just to prevent other things from
      // happening
    } else if (hardware.A && hardware.X && hardware.Y &&
               hardware.Z) { // Toggle Auto-Initialize
      settingChangeCount++;
      changeAutoInit(btn, hardware, controls);
    } else if (hardware.Z && hardware.S && !hardware.A && !hardware.B &&
      ...
      freezeSticks(2000, btn, hardware);
#endif // RUMBLE
    } else if (hardware.A && hardware.B &&
               hardware.S) { // Show current rumble setting
      settingChangeCount++;
#ifdef RUMBLE
      ...
      freezeSticks(2000, btn, hardware);
               ...
               hardware.R) { // C-stick Calibration
      debug_println("Calibrating the C stick");
      whichStick = CSTICK;
      currentCalStep++;
      advanceCal = true;
      freezeSticks(2000, btn, hardware);
    } else if (hardware.A && hardware.X && !hardware.Z &&
               hardware.Du) { // Increase Analog X-Axis Snapback Filtering
      settingChangeCount++;
      adjustSnapback(XAXIS, INCREASE, btn, hardware, controls, gains,
                     normGains);
    } else if (hardware.A && hardware.X && !hardware.Z &&
               hardware.Dd) { // Decrease Analog X-Axis Snapback Filtering
      settingChangeCount++;
      adjustSnapback(XAXIS, DECREASE, btn, hardware, controls, gains,
                     normGains);
    } else if (hardware.A && hardware.Y && !hardware.Z &&
               hardware.Du) { // Increase Analog Y-Axis Snapback Filtering
      settingChangeCount++;
      adjustSnapback(YAXIS, INCREASE, btn, hardware, controls, gains,
                     normGains);
    } else if (hardware.A && hardware.Y && !hardware.Z &&
               hardware.Dd) { // Decrease Analog Y-Axis Snapback Filtering
      settingChangeCount++;
      adjustSnapback(YAXIS, DECREASE, btn, hardware, controls, gains,
                     normGains);
    } else if (hardware.L && hardware.X && !hardware.Z &&
               hardware.Du) { // Increase Analog X-Axis Waveshaping
      settingChangeCount++;
      adjustWaveshaping(ASTICK, XAXIS, INCREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.X && !hardware.Z &&
               hardware.Dd) { // Decrease Analog X-Axis Waveshaping
      settingChangeCount++;
      adjustWaveshaping(ASTICK, XAXIS, DECREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.Y && !hardware.Z &&
               hardware.Du) { // Increase Analog Y-Axis Waveshaping
      settingChangeCount++;
      adjustWaveshaping(ASTICK, YAXIS, INCREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.Y && !hardware.Z &&
               hardware.Dd) { // Decrease Analog Y-Axis Waveshaping
      settingChangeCount++;
      adjustWaveshaping(ASTICK, YAXIS, DECREASE, btn, hardware, controls);
    } else if (hardware.R && hardware.X && !hardware.Z &&
               hardware.Du) { // Increase X-axis Delay
      settingChangeCount++;
      adjustSmoothing(XAXIS, INCREASE, btn, hardware, controls, gains,
                      normGains);
    } else if (hardware.R && hardware.X && !hardware.Z &&
               hardware.Dd) { // Decrease X-axis Delay
      settingChangeCount++;
      adjustSmoothing(XAXIS, DECREASE, btn, hardware, controls, gains,
                      normGains);
    } else if (hardware.R && hardware.Y && !hardware.Z &&
               hardware.Du) { // Increase Y-axis Delay
      settingChangeCount++;
      adjustSmoothing(YAXIS, INCREASE, btn, hardware, controls, gains,
                      normGains);
    } else if (hardware.R && hardware.Y && !hardware.Z &&
               hardware.Dd) { // Decrease Y-axis Delay
      settingChangeCount++;
      adjustSmoothing(YAXIS, DECREASE, btn, hardware, controls, gains,
                      normGains);
    } else if (hardware.R && hardware.A && hardware.Du &&
               !hardware.Z) { // Increase Cardinal Snapping
      settingChangeCount++;
      adjustCardinalSnapping(ASTICK, INCREASE, btn, hardware, controls);
    } else if (hardware.R && hardware.A && hardware.Dd &&
               !hardware.Z) { // Decrease Cardinal Snapping
      settingChangeCount++;
      adjustCardinalSnapping(ASTICK, DECREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.A && hardware.Du &&
               !hardware.Z) { // Increase Analog Scaler
      settingChangeCount++;
      adjustAnalogScaler(ASTICK, INCREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.A && hardware.Dd &&
               !hardware.Z) { // Decrease Analog Scaler
      settingChangeCount++;
      adjustAnalogScaler(ASTICK, DECREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.S && !hardware.A && !hardware.R &&
               !hardware.X &&
               !hardware.Y) { // Show Current Analog Settings (ignore L jump and
                              // L trigger toggle and LRAS)
      showAstickSettings(btn, hardware, controls, gains);
    } else if (hardware.A && hardware.X && hardware.Z &&
               hardware.Du) { // Increase C-stick X-Axis Snapback Filtering
      settingChangeCount++;
      adjustCstickSmoothing(XAXIS, INCREASE, btn, hardware, controls, gains,
                            normGains);
    } else if (hardware.A && hardware.X && hardware.Z &&
               hardware.Dd) { // Decrease C-stick X-Axis Snapback Filtering
      settingChangeCount++;
      adjustCstickSmoothing(XAXIS, DECREASE, btn, hardware, controls, gains,
                            normGains);
    } else if (hardware.A && hardware.Y && hardware.Z &&
               hardware.Du) { // Increase C-stick Y-Axis Snapback Filtering
      settingChangeCount++;
      adjustCstickSmoothing(YAXIS, INCREASE, btn, hardware, controls, gains,
                            normGains);
    } else if (hardware.A && hardware.Y && hardware.Z &&
               hardware.Dd) { // Decrease C-stick Y-Axis Snapback Filtering
      settingChangeCount++;
      adjustCstickSmoothing(YAXIS, DECREASE, btn, hardware, controls, gains,
                            normGains);
    } else if (hardware.L && hardware.X && hardware.Z &&
               hardware.Du) { // Increase C-stick X-Axis Waveshaping
      settingChangeCount++;
      adjustWaveshaping(CSTICK, XAXIS, INCREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.X && hardware.Z &&
               hardware.Dd) { // Decrease C-stick X-Axis Waveshaping
      settingChangeCount++;
      adjustWaveshaping(CSTICK, XAXIS, DECREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.Y && hardware.Z &&
               hardware.Du) { // Increase C-stick Y-Axis Waveshaping
      settingChangeCount++;
      adjustWaveshaping(CSTICK, YAXIS, INCREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.Y && hardware.Z &&
               hardware.Dd) { // Decrease C-stick Y-Axis Waveshaping
      adjustWaveshaping(CSTICK, YAXIS, DECREASE, btn, hardware, controls);
    } else if (hardware.R && hardware.X && hardware.Z &&
               hardware.Du) { // Increase C-stick X Offset
      settingChangeCount++;
      adjustCstickOffset(XAXIS, INCREASE, btn, hardware, controls);
    } else if (hardware.R && hardware.X && hardware.Z &&
               hardware.Dd) { // Decrease C-stick X Offset
      settingChangeCount++;
      adjustCstickOffset(XAXIS, DECREASE, btn, hardware, controls);
    } else if (hardware.R && hardware.Y && hardware.Z &&
               hardware.Du) { // Increase C-stick Y Offset
      settingChangeCount++;
      adjustCstickOffset(YAXIS, INCREASE, btn, hardware, controls);
    } else if (hardware.R && hardware.Y && hardware.Z &&
               hardware.Dd) { // Decrease C-stick Y Offset
      settingChangeCount++;
      adjustCstickOffset(YAXIS, DECREASE, btn, hardware, controls);
    } else if (hardware.R && hardware.A && hardware.Z &&
               hardware.Du) { // Increase C-stick Cardinal Snapping
      settingChangeCount++;
      adjustCardinalSnapping(CSTICK, INCREASE, btn, hardware, controls);
    } else if (hardware.R && hardware.A && hardware.Z &&
               hardware.Dd) { // Decrease C-stick Cardinal Snapping
      settingChangeCount++;
      adjustCardinalSnapping(CSTICK, DECREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.A && hardware.Z &&
               hardware.Du) { // Increase C-stick Analog Scaler
      settingChangeCount++;
      adjustAnalogScaler(CSTICK, INCREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.A && hardware.Z &&
               hardware.Dd) { // Decrease C-stick Analog Scaler
      settingChangeCount++;
      adjustAnalogScaler(CSTICK, DECREASE, btn, hardware, controls);
    } else if (hardware.R && hardware.S && !hardware.A && !hardware.L &&
               !hardware.X &&
               !hardware.Y) { // Show Current C-stick Settings (ignore R jump
                              // and R trigger toggle and LRAS)
    ...
    } else if (hardware.L && hardware.B &&
               hardware.Du) { // Increase L-Trigger Offset
      settingChangeCount++;
      adjustTriggerOffset(LTRIGGER, INCREASE, btn, hardware, controls);
    } else if (hardware.L && hardware.B &&
               hardware.Dd) { // Decrease L-trigger Offset
      settingChangeCount++;
      adjustTriggerOffset(LTRIGGER, DECREASE, btn, hardware, controls);
    } else if (hardware.R && hardware.B &&
               hardware.Du) { // Increase R-trigger Offset
      settingChangeCount++;
      adjustTriggerOffset(RTRIGGER, INCREASE, btn, hardware, controls);
    } else if (hardware.R && hardware.B &&
               hardware.Dd) { // Decrease R-trigger Offset
      settingChangeCount++;
  ...
  } else if (currentCalStep == -1) { // Safe Mode Enabled, Lock Settings, wait
                                     // for safe mode command
  ...
  // Skip stick measurement and go to notch adjust using the start button while
  // calibrating
  if (hardware.S && (currentCalStep >= 0 && currentCalStep < 32)) {
    calibrationSkipMeasurement(
        currentCalStep, whichStick, tempCalPointsX, tempCalPointsY, notchStatus,
        notchAngles, measuredNotchAngles, aStickParams, cStickParams, controls);
  }
  ...
  if ((hardware.A || hardware.L || hardware.R) && advanceCal) {
    if (calibLockout > 0) {
      ...
      calibrationAdvance(controls, currentCalStep, whichStick, tempCalPointsX,
                         tempCalPointsY, undoCal, notchAngles, notchStatus,
                         measuredNotchAngles, aStickParams, cStickParams);
      if (currentCalStep == -1) {
        advanceCal = false;
      }
    }
...
void readSticks(int readA, int readC, Buttons &btn, Pins &pin, RawStick &raw,
                const Buttons &hardware, const ControlConfig &controls,
                const FilterGains &normGains, const StickParams &aStickParams,
                const StickParams &cStickParams, float &dT,
                int &currentCalStep) {
  readADCScale(_ADCScale, _ADCScaleFactor);

  // on Arduino (and therefore Teensy), micros() overflows after about 71.58
  // minutes This is 2^32 microseconds
  static uint32_t lastMicros = micros();
  static uint32_t adcDelta = 0;
  // We increment lastMicros by 1000 each timestep so that we can always try to
  // catch up
  ...
  // #ifndef CLEANADC
  // Read the sticks repeatedly until it's been 1 millisecond since the last
  // iteration This is for denoising and making sure the loop runs at 1000 Hz We
  // want to stop the ADC reading early enough that we don't overrun 1000
  // microseconds
  uint32_t adcCount = 0;
  uint32_t aXSum = 0;
  uint32_t aYSum = 0;
  ...
  do {
    ...
    beforeMicros = afterMicros;
           ...
           (afterMicros - lastMicros > 100000000));
  ...
  */
  ...
  if (micros() - lastMicros >
      1500) { // handle the case that it was synced and now isn't
    lastMicros = micros();
  }
  raw.axRaw = aStickX;
  raw.ayRaw = aStickY;
  ...
  // linearize the analog stick inputs by multiplying by the coefficients found
  // during calibration (3rd order fit)
  ...
  posAx =
      normGains.xSmoothing * shapedAx + (1 - normGains.xSmoothing) * oldPosAx;
  posAy =
      normGains.ySmoothing * shapedAy + (1 - normGains.ySmoothing) * oldPosAy;
  oldPosAx = posAx;
  oldPosAy = posAy;
  ...
  notchRemap(posAx, posAy, &remappedAx, &remappedAy, _noOfNotches, aStickParams,
             currentCalStep, controls, ASTICK);
  notchRemap(posCx, posCy, &remappedCx, &remappedCy, _noOfNotches, cStickParams,
             currentCalStep, controls, CSTICK);
  notchRemap(raw.axLinearized, raw.ayLinearized, &remappedAxUnfiltered,
             &remappedAyUnfiltered, _noOfNotches, aStickParams, 1, controls,
             ASTICK); // no snapping
  notchRemap(raw.cxLinearized, raw.cyLinearized, &remappedCxUnfiltered,
             &remappedCyUnfiltered, _noOfNotches, cStickParams, 1, controls,
             CSTICK); // no snapping
  ...
  skipAHyst = ess::remap(&remappedAx, &remappedAy,
                         controls.extras[ess::extrasEssConfigSlot].config);
#endif
