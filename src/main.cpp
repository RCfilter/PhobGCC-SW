  ...
  while (true) { // main event loop
    ...
    if (_pleaseCommit != 0) {
      if (_pleaseCommit == 1) {
        ...
        _pleaseCommit = 0;
        resetDefaults(SOFT, _controls, _gains, _normGains, _aStickParams,
                      _cStickParams);
      } else if (_pleaseCommit == 3) {
        _pleaseCommit = 0;
        resetDefaults(HARD, _controls, _gains, _normGains, _aStickParams,
                      _cStickParams);
        ...
        calibrationAdvance(_controls, _currentCalStep, whichStick,
                           tempCalPointsX, tempCalPointsY, undoCal, notchAngles,
                           notchStatus, measuredNotchAngles, _aStickParams,
                           _cStickParams);
        ...
        // skip cal (might not actually get used?) (it's available but not
        // shown)
        _pleaseCommit = 255;
        calibrationSkipMeasurement(_currentCalStep, whichStick, tempCalPointsX,
                                   tempCalPointsY, notchStatus, notchAngles,
                                   measuredNotchAngles, _aStickParams,
                                   _cStickParams, _controls);
        ...
        switch (_dataCapture.mode) {
          // make sure that we can write 200 things in a1 (clobbering a2) for
          // longer recording this is used for single-axis recording
          static_assert(&_dataCapture.a1[100] == &_dataCapture.a2[0]);
          static_assert(&_dataCapture.a1Unfilt[100] ==
                        &_dataCapture.a2Unfilt[0]);
        ...
        case CM_REACTION:
          ...
          if (
              ...
              max(max(abs(int(_btn.Ax) - 127), abs(int(_btn.Ay) - 127)),
                  max(abs(int(_btn.Cx) - 127), abs(int(_btn.Cy) - 127))) >=
                  _dataCapture.stickThresh ||
        ...
        case CM_STICK_RISE2:
          ...
          // this records the starting point only the first time, then it sets
          // begin
          static int x0;
          static int y0;
          ...
          if (!_dataCapture.begin && !_dataCapture.done) {
            ...
            _dataCapture.endIndex =
                255; // when it triggers, we freeze startIndex and start
                     // counting up here
            if (_dataCapture.captureStick == ASTICK) {
              ...
              _dataCapture.a1Unfilt[_dataCapture.startIndex] =
                  (uint8_t)(_raw.axUnfiltered + _floatOrigin);
              _dataCapture.a2Unfilt[_dataCapture.startIndex] =
                  (uint8_t)(_raw.ayUnfiltered + _floatOrigin);
            } else {
              ...
              _dataCapture.a1Unfilt[_dataCapture.startIndex] =
                  (uint8_t)(_raw.cxUnfiltered + _floatOrigin);
              _dataCapture.a2Unfilt[_dataCapture.startIndex] =
                  (uint8_t)(_raw.cyUnfiltered + _floatOrigin);
            }
            //                                                                   syxba                       lrz
            _dataCapture.abxyszrl[_dataCapture.startIndex] =
                (_btn.arr[0] & 0b00011111) | ((_btn.arr[1] & 0b01110000) << 1);
          } else {
            if (_dataCapture.endIndex == 255) {
              ...
              if (_dataCapture.captureStick == ASTICK) {
                ...
                if (diffX > _dataCapture.stickThresh ||
                    diffY > _dataCapture.stickThresh) {
              ...
              //                                                                   syxba                       lrz
              _dataCapture.abxyszrl[_dataCapture.startIndex] =
                  (_btn.arr[0] & 0b00011111) |
                  ((_btn.arr[1] & 0b01110000) << 1);
              ...
              if (_dataCapture.captureStick == ASTICK) {
                ...
                _dataCapture.a1Unfilt[_dataCapture.endIndex] =
                    (uint8_t)(_raw.axUnfiltered + _floatOrigin);
                _dataCapture.a2Unfilt[_dataCapture.endIndex] =
                    (uint8_t)(_raw.ayUnfiltered + _floatOrigin);
              } else {
              ...
              //                                                                 syxba                       lrz
              _dataCapture.abxyszrl[_dataCapture.endIndex] =
                  (_btn.arr[0] & 0b00011111) |
                  ((_btn.arr[1] & 0b01110000) << 1);
              // increment endIndex
        ...
        case CM_STICK_RISE:
          ...
          // it should record for 150 ms after detection (saving 50 from before
          // detection)
          ...
          if (_dataCapture.captureStick == ASTICK &&
              _dataCapture.whichAxis == XAXIS) {
            _dataCapture.a1[_dataCapture.endIndex] = _btn.Ax;
            _dataCapture.a1Unfilt[_dataCapture.endIndex] =
                (uint8_t)(_raw.axUnfiltered + _floatOrigin);
          } else if (_dataCapture.captureStick == ASTICK &&
                     _dataCapture.whichAxis == YAXIS) {
          ...
          // in this case, we want the stick's raw value to be out of the
          // deadzone
          if (!_dataCapture.begin && !_dataCapture.done) {
            if (_dataCapture.captureStick == ASTICK &&
                _dataCapture.whichAxis == XAXIS) {
          ...
          // stop if done
          if (_dataCapture.begin &&
              _dataCapture.startIndex == _dataCapture.endIndex) {
            // calculate the dashback chance
            int counter = 0;
            for (int i = 0; i < 200; i++) {
              if (fabs(
                      _dataCapture.a1[(i + _dataCapture.startIndex + 1) % 200] -
                      _intOrigin) >= 23) {
                if (fabs(_dataCapture
                             .a1[(i + _dataCapture.startIndex + 1) % 200] -
                         _intOrigin) >= 64) {
                  break;
                }
                counter++;
            ...
            _dataCapture.percents[0] =
                fmax(0, (1 - (counter / 16.666667f)) * 100);
        ...
        case CM_STICK_FALL:
          ...
          if (!_dataCapture.begin && !_dataCapture.triggered &&
              !_dataCapture.done) {
            if (_dataCapture.captureStick == ASTICK &&
                _dataCapture.whichAxis == XAXIS) {
        ...
        case CM_STICK_PIVOT:
          ...
          // check for initial waiting state
          // in this case, we want the stick's raw value to be >=80 (dash
          // threshold) also record which side
          if (!_dataCapture.begin && !_dataCapture.triggered &&
              !_dataCapture.done) {
            if (_dataCapture.captureStick == ASTICK &&
                _dataCapture.whichAxis == XAXIS) {
        ...
        case CM_TRIG:
          ...
          if (_dataCapture.begin &&
            ...
            for (int i = 0; i < 200; i++) {
              ...
              if (digital) {
                break; // if digital is active on the same ms as analog or
                       // before, we're good
              }
              if (analog >= 43) {
                counter++;
        ...
        case CM_PRESS:
          ...
          if (_dataCapture.begin && !_dataCapture.done) {
            //                                                                 syxba                       lrz
            _dataCapture.abxyszrl[_dataCapture.endIndex] =
                (_btn.arr[0] & 0b00001111) | ((_btn.arr[1] & 0b01110000) << 1);
            _dataCapture.axaycxcyrl[_dataCapture.endIndex] =
                (0b0000'0001 *
                 (abs(int(_btn.Ax) - 127) >= _dataCapture.stickThresh)) |
                (0b0000'0010 *
                 (abs(int(_btn.Ay) - 127) >= _dataCapture.stickThresh)) |
                (0b0000'0100 *
                 (abs(int(_btn.Cx) - 127) >= _dataCapture.stickThresh)) |
    ...
    // check to see if we are calibrating
      ...
      if (whichStick == ASTICK) {
        if (_currentCalStep >= _noOfCalibrationPoints) { // adjust notch angles
          adjustNotch(_currentCalStep, _dT, ASTICK, measuredNotchAngles,
                      notchAngles, notchStatus, _btn, _hardware);
          if (_hardware.Y || _hardware.X ||
              (_btn.B)) { // only run this if the notch was adjusted
            ...
            cleanCalPoints(tempCalPointsX, tempCalPointsY, notchAngles,
                           cleanedPointsX, cleanedPointsY, notchPointsX,
                           notchPointsY, notchStatus);
            // linearize again
            linearizeCal(cleanedPointsX, cleanedPointsY, cleanedPointsX,
                         cleanedPointsY, _aStickParams);
            // notchCalibrate again to update the affine transform
            notchCalibrate(cleanedPointsX, cleanedPointsY, notchPointsX,
                           notchPointsY, _noOfNotches, _aStickParams);
          }
        } else { // just show desired stick position
          displayNotch(_currentCalStep, true, _notchAngleDefaults, _btn);
        }
        readSticks(true, false, _btn, _pinList, _raw, _hardware, _controls,
                   _normGains, _aStickParams, _cStickParams, _dT,
                   _currentCalStep);
      } else {                                           // WHICHSTICK == CSTICK
        if (_currentCalStep >= _noOfCalibrationPoints) { // adjust notch angles
          adjustNotch(_currentCalStep, _dT, CSTICK, measuredNotchAngles,
                      notchAngles, notchStatus, _btn, _hardware);
          if (_hardware.Y || _hardware.X ||
              (_btn.B)) { // only run this if the notch was adjusted
            // clean full cal points again, feeding updated angles in
            ...
            // linearize again
            linearizeCal(cleanedPointsX, cleanedPointsY, cleanedPointsX,
                         cleanedPointsY, _cStickParams);
            // notchCalibrate again to update the affine transform
            notchCalibrate(cleanedPointsX, cleanedPointsY, notchPointsX,
                           notchPointsY, _noOfNotches, _cStickParams);
          }
        } else { // just show desired stick position
          displayNotch(_currentCalStep, false, _notchAngleDefaults, _btn);
        }
        readSticks(false, true, _btn, _pinList, _raw, _hardware, _controls,
                   _normGains, _aStickParams, _cStickParams, _dT,
                   _currentCalStep);
      }
    } else if (running) {
      // if not calibrating read the sticks normally
      readSticks(true, true, _btn, _pinList, _raw, _hardware, _controls,
                 _normGains, _aStickParams, _cStickParams, _dT,
                 _currentCalStep);
    }
...
int main() {
  ...
  // Read settings; true = don't lock out the core because running it on core 1
  // will freeze
  ...
  if (numberOfNaN > 10) { // by default it seems 16 end up unitialized on pico
    resetDefaults(FACTORY, _controls, _gains, _normGains, _aStickParams,
                  _cStickParams, true);
  ...
  Pins pinList{// not actually necessary for any of the rp2040 read functions
               .pinLa = 0, .pinRa = 0, .pinL = 0,  .pinR = 0,  .pinAx = 0,
               .pinAy = 0, .pinCx = 0, .pinCy = 0, .pinRX = 0, .pinTX = 0,
               .pinDr = 0, .pinDu = 0, .pinDl = 0, .pinDd = 0, .pinX = 0,
               .pinY = 0,  .pinA = 0,  .pinB = 0,  .pinZ = 0,  .pinS = 0};
  ...
  initializeButtons(_pinList, _btn, _controls.lTrigInitial,
                    _controls.rTrigInitial);
  ...
  if (_hardware.Z) { // hold Z on powerup for PhobVision
    ...
    set_sys_clock_khz(
        1000 * 250,
        true); // overclock to 250 khz, to alleviate performance issues
  }
  ...
  if (_hardware.Z) {
    ...
    const int version = SW_VERSION;
#endif // BUILD_DEV
    videoOut(_pinDac0, _btn, _hardware, _raw, _controls, _aStickParams,
             _cStickParams, _dataCapture, _sync, _pleaseCommit, _currentCalStep,
             version);
  } else {
    enterMode(_pinTX, _pinRumble, _pinBrake, _rumblePower, buttonsToGCReport);
  }
}
